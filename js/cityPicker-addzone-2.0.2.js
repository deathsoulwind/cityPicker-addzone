...
(function ($, window) {
    var $selector;
    var grade = ['area','province', 'city', 'district', 'street'];
    var defaults = {
        dataJson: null,
        selectpattern: [
            {
    ...
    var effect = {
        ...
        obtain: function (event) {
            ...
            // 判断类型
            if (config.renderMode) {
                /*custom s zcf 增加了大区级别，index < 3改为4*/
                //给选中的级-添加值和文字
                ...
                index < 4 ? $target.next().trigger('change') : '';
            }
            /*custom s zcf 增加了大区级别，config.level === 4改为5*/
            // 开启四级联动，添加四级城市
            if (config.level === 5 && index === 2) {
                self.getStreet(id);
            }
            /* custom e */
            // 选择完后执行的回调
